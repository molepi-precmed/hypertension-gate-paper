...
<!-- Association of transcript levels in whole blood [@farzeenNovelKnownGenetic2025] **(NOTE: finish this)**...  -->
...
Figure \ref{fig:figure} (B), shows a heatmap of correlations between the  `r coregenes.eqtl.strict[, .N]` eQTL GATE scores presented in Table \ref{tab:transeqtls}. The eGATE scores are ordered by hierarchical clustering on the squared correlations so that highly correlated scores appear in diagonal blocks.   This shows a cluster of highly correlated scores for genes that are expressed on B cells, including _BANK1_, _COBLL1_ and _MEF2C_. 
...
As the SNP effects on blood pressure are highly polygenic, so also the genes on which their _trans_-effects coalesce are more numerous, and have weaker effects than in our previous studies of autoimmune diseases that are relatively oligogenic.  The largest effect that we identify is for _IL17B_, where an increase of 1 SD in the measured protein level lowers blood pressure by only 1.2 mmHg and an increase of 1 SD in the GATE score (which accounts for only **NOTE: XX%** of the variance in the measured protein level) lowers blood pressure by 0.1 mmHg. 
...
* _TREM1_ encodes a receptor belonging to the Ig superfamily that is expressed on myeloid cells. This protein amplifies neutrophil and monocyte-mediated inflammatory responses **NOTE: ADD citation**

* _CLEC4C_ encodes a receptor for asialo-galactosyl-oligosaccharides that inhibits interferon production by plasmacytoid dendritic cells. **NOTE: ADD citation**

* _CD209_ (alias dendritic cell-specific intercellular adhesion molecule-3–grabbing nonintegrin, DC-SIGN), is a C-type lectin receptor that is expressed primarily on subsets of dendritic cells and certain macrophages. It recognizes and binds to specific carbohydrate moieties—such as high-mannose structures and fucose-containing glycans expressed by  pathogens. **NOTE: ADD citation**
...
.note_lines <- c(
  ...
  "(c) Association of MAP with measured protein levels in UKB-PPP: \\textbf{\\texttt{+}} indicates association at $P < 10^{-4}$ with effect direction consistent with GATE score effect on MAP; \\textbf{\\texttt{-}} indicates opposite direction; \\texttt{0} indicates no association.",
...
if (params$output_format == "pdf") {
  ...
  knitr::asis_output(paste0(as.character(.tbl_prot_eqtl),
                             "\n\\newpage\n",
...
.plain_nearby_x <- function(dt, gwas_genes_list) {
  vapply(seq_len(nrow(dt)), function(i) {
    gwas_genes <- gwas_genes_list[[i]]
    nearby <- trimws(strsplit(as.character(dt$nearby_genes[i]), ",")[[1]])
    nearby <- nearby[nearby != "" & nearby != "."]
    if (!length(nearby)) return("")
    if (length(gwas_genes)) {
      # Excel-friendly "bold": render gene symbols in Unicode mathematical bold
      .to_unicode_bold <- function(x) {
        x <- as.character(x)
        s <- strsplit(x, "", fixed = TRUE)[[1]]
        out <- vapply(s, function(ch) {
          code <- utf8ToInt(ch)
          # Uppercase A-Z
          if (code >= 65 && code <= 90) return(intToUtf8(0x1D400 + (code - 65)))
          # Lowercase a-z
          if (code >= 97 && code <= 122) return(intToUtf8(0x1D41A + (code - 97)))
          # Digits 0-9
          if (code >= 48 && code <= 57) return(intToUtf8(0x1D7CE + (code - 48)))
          ch
        }, character(1))
        paste(out, collapse = "")
      }
      paste(ifelse(nearby %in% gwas_genes, .to_unicode_bold(nearby), nearby), collapse = ", ")
    } else paste(nearby, collapse = ", ")
  }, character(1))
}
...
writexl::write_xlsx(tbl_transeqtls, "output/transeqtls_clumps.xlsx")
...
writexl::write_xlsx(tbl_transpqtls, "output/transpqtls_clumps.xlsx")
```
