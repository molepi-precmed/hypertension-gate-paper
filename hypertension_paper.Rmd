...
To obtain linkage disequilibrium (LD)-adjusted SNP weights, we premultiplied the univariate regression coefficients by the inverse genotype covariance matrix in a reference panel of European ancestry. For each individual, the GATE score for a transcript (eQTL GATE score) or protein (pQTL GATE score) was then calculated as the dot product of the genotypes in the target cohort and these LD-adjusted weights at _trans_-QTLs, representing the genetically determined expression or protein level from all _trans_-QTLs. The locus-specific contributions are provided in Supplementary Data files.  SNPs corresponding to _cis_-QTLs for the gene were aggregated separately into a _cis_ score, independent from the GATE score. 
...
Figure \ref{fig:figure} (B), shows a heatmap of correlations between the  `r coregenes.eqtl.strict[, .N]`  eQTL GATE scores that have effective number of _trans_-QTLs at least 8 and are associated with `r phenoname` at $P<10^{-5}$.  The GATE scores are ordered by hierarchical clustering on the squared correlations so that highly correlated scores appear in diagonal blocks.   This shows a cluster of highly correlated scores for genes that are expressed on B cells, including _BANK1_, _COBLL1_ and _MEF2C_. 

Figure \ref{fig:figure} (D) shows that these GATE scores are co-regulated by the _SH2B3_ locus on chromosome 12, which appears to be a "peripheral master regulator".  This Figure shows no clear evidence for core genes on which many GWAS hit genes coalesce via _trans_ effects.  This is likely to reflect the limitations of the eQTLGen summary statistics, in which most SNPs were not tested for _trans_-effects. 

Table \ref{tab:coregenes.eqtls.strict} shows that the GATE scores that form the cluster of B cell genes are inversely associated with MAP.   The GATE score association with MAP was supported by a _cis_-eQTL association for three of the putative core genes, and supported by reported GWAS hit within 200 kb for `r coregenes.eqtl.strict[!is.na(reported.genes),.N]` of the putative core genes, although most of these GWAS hits had not been attributed to a nearby gene in the original GWAS.

## Association of mean arterial pressure with GATE scores for circulating proteins
Figure \ref{fig:figure} (C) shows a heatmap of correlations betweeen the `r coregenes.pqtl.strict[, .N]` pQTL GATE scores that have effective number of _trans_-pQTLs at least 20 and are associated with `r phenoname` at $P<10^{-5}$.  Only two GATE scores -- for _CNTN3_ and _CKB_ -- are highly correlated.  For the other genes, it is less likely that GATE score associations with blood pressure can be explained by pleiotropic effects.   
...
Table \ref{tab:coregenes.pqtls.strict} shows that the most strongly associated pGATE scores was for *IL17RB*.  Of the `r coregenes.pqtl.strict[, .N]` putative core genes shown, `r coregenes.pqtl.strict[!is.na(reported.genes),.N]` had a reported GWAS hit within 200kb most of which could not be localised to any gene in the originating GWAS.

A full listing of all eGATE and pGATE scores (N=`r coregenes.pqtl[, .N]+coregenes.eqtl[, .N]`) that were associated with MAP at $P<10^{-5}$ but where the effective number of loci threshold is relaxed to $\ge 5$ is available as a Supplementary file.  

## Dose-response (Mendelian randomization) analysis
Supplementary Tables \ref{tab:mrhevo.eqtl} and \ref{tab:mrhevo.pqtl} show the results of the dose-response analysis for putative core genes listed in Table \ref{tab:coregenes.eqtls.strict} and Table \ref{tab:coregenes.pqtls.strict}. 
Of the eQTL GATE scores for putative core genes only _ABCA1_ showed a dose-response relationship at $P<0.01$.  As the statistical power to detect a dose-response relationship depends upon the number of trans-QTLs, the tests for eQTL GATE scores have relatively low statistical power.  Of the pQTL GATE scores for putative core genes, `r all.evidence.tbl[mr.validated=="+"&source=="pQTL",.N]` of `r coregenes.pqtl.strict[, .N]` genes in Table \ref{tab:coregenes.pqtls.strict} showed a dose-response effect on mean arterial pressure at $P < 0.01$).  Figure \ref{fig:mrpqtl} shows scatter plots of the dose-response relationship betwen the effects of each _trans_-pQTL on protein levels and the effects of each _trans_-pQTL on blood pressure, for the associations for the top 6 genes that show a dose-response relationship: _IL17RB_, _CEACAM16_, _PTPRF_, _CKB_, _TCL1A_ and _CNTN3_.
...
xls.tbl <- all.evidence.tbl[, .(gene_symbol, gwas.validated, mr.validated, protein.validated, model.validated, drug.validated)]
...
sym_cols <- c("gwas.validated", "mr.validated", "protein.validated", "model.validated", "drug.validated")
for (j in sym_cols) {
...
.note_lines <- c(
  ...
  "(f) Drug in development that targets the protein encoded by the gene or its ligand/receptor alters blood pressure in humans."
)
.note_latex <- paste0(
...
if (params$output_format == "pdf") {
  ...
  table.data[, `:=`(p_trans=NULL, p_cis=NULL)],
             ...
             col.names=c("pQTL study", "Gene", "Chr", "Start position (Mb)",
                         ...
                         "$P$ value",
                         "Reported GWAS hit"),
             caption=("Additional putative core genes defined by an association $P < 10^{-5}$ and effective \\textit{trans}-pQTLs contributing to the pGATE score not less than five but less than twenty."),
